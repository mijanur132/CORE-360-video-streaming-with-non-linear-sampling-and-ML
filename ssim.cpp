...
/**
	...
	*/
double getssim(Mat img_src, Mat img_compressed, int block_size, bool show_progress = false)
{
	double ssim = 0;
...
void compute_quality_metrics(char * file1, char * file2, int block_size)
{
	...
	cout << file1 << endl;
	// Loading pictures
	img_src = imread(file1, IMREAD_GRAYSCALE);
	img_compressed = imread(file2, IMREAD_GRAYSCALE);
	...
	double ssim_val = getssim(img_src, img_compressed, block_size);

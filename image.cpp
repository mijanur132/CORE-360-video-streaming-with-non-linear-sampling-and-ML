...
int ChngReriCS = 0;
int timePassed = 0;
float nextDlChunkSecVar=1000;
float DlTimeVariation = 0;
float variationConstant = 0;
float variationConstant2 = 0;
...
{	
	...
	Mat frame;
	cout <<"file to be dl:"<< filename << endl;
	...
	while((j < fps*chunkD+extraFrame))  //&& requestedChunk2DL <=chunkN
	{
	...
	DlTimeVariation = (1 - variationConstant) * DlTimeVariation + variationConstant * (elapsedTimeMS-nextDlChunkSecVar );
	nextDlChunkSecVar = (1-variationConstant2)* nextDlChunkSecVar+ variationConstant2* elapsedTimeMS;
	nextDlChunkSecVar =(1-0.5)* nextDlChunkSecVar +  0.5*DlTimeVariation;
	cout<<"varc: " << variationConstant << "varc2: " << variationConstant2 << endl;
	//nextDlChunkSecVar = 0.1*nextDlChunkSecVar+0.9*elapsedTimeMS;

	if (nextDlChunkSecVar > 4000) {
		nextDlChunkSecVar = 4000;
	}
	cout << "fi:............................................................ " << j << " of Chnk: " << chunkN << endl;
...
string getChunkNametoReqnRefCamOptimizedNY(PPC& camera1, M33& reriCS, string srcBaseAddr, int chunkN, V3 cameraDirection, int tiltAngle)
{
	ERI eri(10, 10); //size does not matter here
	float latt = eri.GetXYZ2Latitude(cameraDirection);
	float longg = eri.GetXYZ2Longitude(cameraDirection);
	...
	oss1 << srcBaseAddr << "_" << chunkN << "_" << reqtilt << "_" << reqpann << ".avi.avi";
	PPC camera = camera1;
	//cout <<"ref cam direction"<< camera1.GetVD() << endl;
	camera.Pan(reqpann);
	camera.Tilt(reqtilt);
...
{
	futureFrame = 0; accuracy = 100; //if you want to have no prediction, just take current VD, also accuracy needs to be 100 // palash delete
...
void testDownloadVideoHttp4thSecVarWithFatCoRENY(char* srcBaseAddr, char* bwLog, char* hmdFileName,  int nextDlChunkSec, int extraSec,  char* fileSizeName, int FatCoRE, int accuracy, float varc, float varc2)
{
	variationConstant = varc;
	variationConstant2 = varc2;
	//int accuracy = 100;
	...
	vector <int> srRealT;	vector <int> srMinRealT; vector <int> frRealT; vector <int> totalSizes; int totalSizesInt;
	struct samplingvar svar; Mat ret1;	Path path1;
	M33 reriCS;	M33 reriCScopy;
	int compressionFactor = 5;	int frameStarted = 0;		int chunkN = 0;	int fps = 30;
	float var[10];	float howManySecondsfor4thSec = extraSec;	int extraFrameN = 5 * extraSec;	int mxChunkN = 14;	int chunkD = 4;  //
	Mat tempP;
	int futureFrame = (nextDlChunkSec) * fps;
	vector<float> chunkSizeKB; vector<string>chunkName;
	int mahimahiPackSize = 1400;
	...
	int frameLen = var[0];	int frameWidth = var[1];		float hfov = 90.0f;
	float corePredictionMargin = 1;	int w = frameLen * hfov / 360; 	int h = frameWidth * hfov / 360;
	...
	chunkN++;
	filename = getChunkNametoReqnRefCamOptimizedNY(refCam, reriCS, srcBaseAddr, chunkN, VD, tiltAngle);
	cout << "ReqF=" << filename << endl;
	...
	if (samplingValueCalculate == 1)
	{
		...
		if (namePos <= chunkName.size())
		{
			...
			totalSizes.push_back(chunkSizeKB[namePos]);
	...
	auto playStart = std::chrono::high_resolution_clock::now();
	DownLoadChunk4thSecVar(filename, chunkD, fps, extraFrameN, bwTimeNeeded, chunkN);
	requestedChunk2DL = chunkN;
	int chunkN4extraPartCalc = 1;
	int chunkN4extraPart = 1;
	...
	while (condition)
	{
		...
		{
			//nextDlChunkSecVar = 1000;
			//reqTime.push_back(1000);
			...
			filename = getChunkNametoReqnRefCamOptimizedNY(refCam, reriCScopy, srcBaseAddr, reqChunkN, VD, tiltAngle);
			//outputFileName << filename << endl;
			cout << "New Req. " << filename << endl;
			finish1 = std::chrono::high_resolution_clock::now();
			elapsed1 = finish1 - start;
			...
			cout << "Later DLs req Time:------>" << bwDuration << " bwStartIndx---> " << bwIglobal << " bwEndIndex-->" << bwIglobal + bwTimeNeeded << endl;
			requestedChunk2DL =reqChunkN;
			futures.push_back(std::async(std::launch::async, DownLoadChunk4thSecVar, filename, chunkD, fps, extraFrameN, bwTimeNeeded, reqChunkN));
		}

		if (currentChunk > 5 & playIndex == 89)
		{
			cout << "erase................................................................................................................." << endl;
			for (int j = 0; j < currentChunk-5; j++)
			{
				for (int i = 0; i < fps*chunkD+extraFrameN; i++) {
					Mat tempx;
					frameQvec[j][i] = tempx;
		...
		if (currentChunk > 2 & playIndex == 89)
		{
			cout << "erase................................................................................................................." << endl;
			for (int j = 0; j < currentChunk - 2; j++)
			{
				for (int i = 0; i < fps * chunkD -1; i++) {
					Mat tempx;
					frameQvec[j][i] = tempx;
				}
			}
		...
		if (playIndex == 88) { chunkN4extraPartCalc = 1; }
		...
		if (currentChunk <= NextChunkDownloaded)
		{
			...
			Mat frame;	frame = frameQvec[currentChunk][playIndex].clone();
			...
			if ((playIndex) == (int)(fps * (chunkD - nextDlChunkSecVar / 1000.0f)))
			{
				...
				if (bwTimeNeeded > 200000 * (fps * chunkD) * 33)
				{
					reqChunkN = reqChunkN + 1;
					//NextChunkDownloaded++;

					filename = getChunkNametoReqnRefCamOptimizedNY(refCam, reriCScopy, srcBaseAddr, reqChunkN, VD, tiltAngle);
					cout << "2New Req. " << filename << endl;
					finish1 = std::chrono::high_resolution_clock::now();
					elapsed1 = finish1 - start;
					...
					if (namePos <= chunkName.size())
					{
						cout << " 2namePos:" << namePos << " " << chunkName[namePos] << " " << chunkSizeKB[namePos] << endl;
						packetNeeded = chunkSizeKB[namePos] * 1024 / mahimahiPackSize;
						totalSizes.push_back(chunkSizeKB[namePos]);
						//bwTimeNeeded = byteVec[bwIglobal + packetNeeded] - byteVec[bwIglobal]; //in ms
						...
						cout << "2NamePos: " << namePos << " Name:" << chunkName[namePos] << " size:" << chunkSizeKB[namePos] << " chunKNamesize:" << chunkName.size() << endl;
						packetNeeded = 2000 * 1024 / mahimahiPackSize;
						cout << "2file Not found:........................................................................................................................................... " << filename << endl;
						STOP;
						filename = "http://127.0.0.5:80/3vid2crf3trace/4s6s/mobisys/crf30/30_diving.AVI6_1_10_0.avi";
						//bwTimeNeeded = byteVec[bwIglobal + packetNeeded] - byteVec[bwIglobal]; //in ms
					}					
				...
				requestedChunk2DL =reqChunkN;
				cout << "Later DLs requred Time:------>" << bwTimeNeeded << " bwStartIndx---> " << bwIglobal << " bwEndIndex-->" << bwIglobal + bwTimeNeeded << endl;
				futures.push_back(std::async(std::launch::async, DownLoadChunk4thSecVar, filename, chunkD, fps, extraFrameN, bwTimeNeeded, reqChunkN));
		...
		{
			after90Index++; //0->
			if (chunkN4extraPartCalc==1)
			{
				chunkN4extraPart = (frameCount) / (fps * chunkD);
				if (chunkN4extraPart > NextChunkDownloaded) {
					chunkN4extraPart = NextChunkDownloaded;					
				}
				chunkN4extraPartCalc =0;
				cout << "chunkN4extraPart:.................................................. " << chunkN4extraPart << endl;
			}
			cam_index = path1.GetCamIndex(frameCount, fps, cam_index);
			int index = nonUniformList[after90Index];
			int index2 = nonUniformList[after90Index + 1];
			...
			for (int i = 0; i < diffIndex; i++)
			{
				...
				cout << "119CuntChunkN=" << currentChunk - 1 << " frmcount=" << frameCount << " plCnt=" << playIndex <<"diffIndx="<<diffIndex<<"currFr: "<<currentFR<<" after90indx="<<  after90Index << " crntChnk: " << NextChunkDownloaded << endl;
				cam_index = path1.GetCamIndex(frameCount, fps, cam_index);
				Mat frame1;
				//frame1 = tempP;
				float frameRateTemp = diffIndex;
				
				//displayImage(frameQvec[currentChunk - 1][(fps * chunkD - 1) + after90Index]);
				//waitKey(500);
				
				if (samplingValueCalculate == 1)
				{

					path1.CRERI2ConvOptimizedWithSamplingRateVec(frameQvec[chunkN4extraPart][(fps * chunkD - 1) + after90Index], var, eri, reriCS, convPixels, srVec, compressionFactor, path1.cams[cam_index], refCam);
	...
	if (samplingValueCalculate == 1)
	{
		...
		float avgSrNewMethod = 0;
		cout << "SR size:" << srVec.size() << " srrealT: "<<srRealT.size()<<" frRealT:"<<frRealT.size()<< endl;
		...
		ofstream outputSRx;
		outputSRx.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/Core_FR_SR_all.txt", std::ios::out | std::ios::app);
		outputSRx << srcBaseAddr << hmdFileName << bwLog << endl;
		for (int i = 0; i < srRealT.size(); i++)
		{
			outputSRx << "file:" << srcBaseAddr << ",srAAvgRealTime:" << srRealT[i] << ",srMinRealTime:" << srMinRealT[i] << ",frRealTime:" << frRealT[i] << endl;
		...
		ofstream outputSR;
		outputSR.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/CoRE_detailed_all.txt", std::ios::out | std::ios::app);
		outputSR << srcBaseAddr << hmdFileName << bwLog << endl;
		for (int i = 0; i < srRealT.size(); i++)
		{
			//outputSR << "file:" << srcBaseAddr << ",srAAvgRealTime:" << srRealT[i] << ",srMinRealTime:" << srVec[i] << ",frRealTime:" << frRealT[i] << endl;
			if (srRealT[i] > 1 && srRealT[i]<=101)
			{
				avgSrNewMethod =avgSrNewMethod+ srRealT[i];
				newMethodCount++;
			}

		}
		float newMethodAvgSr = avgSrNewMethod / newMethodCount;
		outputSR.close();
		...
		ofstream output;
		output.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/Ny_CoRE_concise.txt", std::ios::out | std::ios::app);
		output << "File," << dest << "," << dest1 << "," << dest2 << "," << nextDlChunkSec << ",avgSR," << avg << ",avgSRNew, " << newMethodAvgSr << ",minSR," << min << ",MinFR," << minFR << ",avgFR," << avgFR << ",totalDelay," << delayTotal << endl;
		output.close();
		cout << "File," << dest << "," << dest1 << "," << dest2<< "," << nextDlChunkSec << ",avgSR," << avg << ",avgSRNew, " << newMethodAvgSr << ",minSR," << min << ",MinFR," << minFR << ",avgFR," << avgFR << ",totalDelay," << delayTotal << endl;

		totalSizesInt = 0;
	

		ofstream output2;
		output2.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/CoRE_Size_concise.txt", std::ios::out | std::ios::app);

		for (int i = 0; i < totalSizes.size(); i++)
		{
			totalSizesInt = totalSizesInt + totalSizes[i];
			//cout <<"totalSizes[i]: "<< totalSizes[i] << endl;
		}


		output2 << srcBaseAddr << bwLog << hmdFileName << "total chunk: " << totalSizes.size() << " totalSize: " << totalSizesInt << endl;
		output2.close();
...
void testDownloadVideoHttp4thSecVarWithFatCoRE(char* srcBaseAddr, char* bwLog, char* hmdFileName, int nextDlChunkSec, int extraSec, char* fileSizeName, int FatCoRE, int accuracy, float varc, float varc2)
{
	variationConstant = varc;
	variationConstant2 = varc2;
	//int accuracy = 100;
	cout << srcBaseAddr << endl;
	cout << bwLog << endl;
	cout << hmdFileName << endl;
	cout << fileSizeName << endl;
	int Size = 1500;
	int samplingValueCalculate = 1;
	auto start = std::chrono::high_resolution_clock::now();
	...
	Mat tempP;
	int futureFrame = (nextDlChunkSec)*fps;
	vector<float> chunkSizeKB; vector<string>chunkName; vector<string> reqChunks;
	int mahimahiPackSize = 1400;
	cout << fileSizeName << endl;
	ifstream  file1(fileSizeName);
	if (!file1)
	...
	if (samplingValueCalculate == 1)
	{
		...
		ofstream output;
		output.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/CoRE_concise.txt", std::ios::out | std::ios::app);
		output << "File," << dest << "," << dest1 << "," << dest2 << "," << nextDlChunkSec << ",avgSR," << avg << ",avgSRNew, " << newMethodAvgSr << ",minSR," << min << ",MinFR," << minFR << ",avgFR," << avgFR << ",totalDelay," << delayTotal << endl;
		output.close();
		cout << "File," << dest << "," << dest1 << "," << dest2 << "," << nextDlChunkSec << ",avgSR," << avg << ",avgSRNew, " << newMethodAvgSr << ",minSR," << min << ",MinFR," << minFR << ",avgFR," << avgFR << ",totalDelay," << delayTotal << endl;
		...
		ofstream reqChunk;
		reqChunk.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/CoRE_chunkRequested.txt", std::ios::out | std::ios::app);
		for (int i = 0; i < reqChunks.size(); i++)
		{
			reqChunk << reqChunks[i] << endl;
		}
...
void testDownloadVideoHttp4thSecVar(char* srcBaseAddr, char* bwLog, char* hmdFileName, int nextDlChunkSec, int extraSec, int Size)
{
	...
	if (samplingValueCalculate == 1)
	{
		...
		ofstream outputSR;		
		outputSR.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/Core_FR_SR_all.txt", std::ios::out | std::ios::app);
		outputSR << srcBaseAddr << hmdFileName << bwLog << endl;
		for (int i = 0; i < srRealT.size(); i++)
		{
...
void getTilesNumber2req2QL(vector<int>& tileBitMap, PPC camera, ERI& eri, int m, int n)
{
	...
	for (int v = 0; v < camera.h; v++)
	{
		for (int u = 0; u < camera.w; u++)
		{
			...
			int Ytile = floor(pixelI * n / eri.h);
			int vectorindex = (Ytile)*m + Xtile;
			if (tileBitMap.at(vectorindex) == 0)
			{
				tileBitMap.at(vectorindex) = 2;
			}
...
void testTileNewModels(char* srcBaseAddr, char*srcBaseAddr2, char* bwLog, char* size, char* hmdFileName, int nextDlChunkSec, int rowM, int colN, int tileModel)
{
	int fovPlusOrno = tileModel;   //1 fovPlus+360, 0 fov+360,
	float corePredictionMargin = 1;
	int mxChunkN = 59;
	...
	if (fovPlusOrno == 0)
	{
		chunkBwDivider = 5 + 19 * 14 / 34;
	}
	if (fovPlusOrno == 1)
	{
		chunkBwDivider = 12 + 12 * 14 / 34;
	}
	...
	for (int i = 0; i < tileRowN * tileColN; i++)
	{
		...
		if (tileBitMap[i] == 1)
		{
			...
			cout << namePos << " " << chunkName[namePos] << " " << chunkSizeKB[namePos] << endl;
			totalSize.push_back(chunkSizeKB[namePos]);
			packetNeeded =chunkSizeKB[namePos] * 1024 / mahimahiPackSize;
			bwTimeNeeded =chunkBwDivider * (byteVec[bwIglobal + packetNeeded] - byteVec[bwIglobal]); //in ms
			cout << "time: " << bwDuration << " Index4Time= " << bwIglobal << " reqTime=" << bwTimeNeeded << endl;
			futures.push_back(std::async(std::launch::async, DownLoadTilesChunk, filename, chunkN, chunkD, i, fps, bwTimeNeeded));
		}
		cout << i << "--" << tileBitMap[i] << endl;
		if (tileBitMap[i] == 0)
		{
			filename = getChunkNametoReq4mTiles(srcBaseAddr2, chunkN, i);
			std::ostringstream oss1;
			oss1 << filename  << ".avi";
			filename = oss1.str();
			cout << filename << endl;
			...
			cout << namePos << " " << chunkName[namePos] << " " << chunkSizeKB[namePos] << endl;
			totalSizeL.push_back(chunkSizeKB[namePos]);
			packetNeeded = chunkSizeKB[namePos] * 1024 / mahimahiPackSize;
			bwTimeNeeded = chunkBwDivider * (byteVec[bwIglobal + packetNeeded] - byteVec[bwIglobal]); //in ms
			cout << "time: " << bwDuration << " Index4Time= " << bwIglobal << " reqTime=" << bwTimeNeeded << endl;
			futures.push_back(std::async(std::launch::async, DownLoadTilesChunk, filename, chunkN, chunkD, i, fps, bwTimeNeeded));
	...
	totalChunkReq.push_back(reqTiles[chunkN].size());
	int condw = 1;
	while (condw)// (NextChunkDownloaded < reqTiles[chunkN].size() - 1)
	{
		//displayImage33ms();
		displayImage(firstScene);
		timePassed = timePassed + 30.0f;
	...
	while (tileCondition)
	{
		...
		if ((playIndex) == 1 && currentChunk<mxChunkN)
		{
			...
			int reqChunkN = currentChunk + 1;
			bitMapVec[reqChunkN] = tileBitMap2;
			...
			for (int i = 0; i < tileRowN * tileColN; i++)
			{
				...
				if (reqChunkN <= mxChunkN)
				{
					if (tileBitMap2[i] == 1)
					{
						...
						reqTiles[reqChunkN].push_back(i);
						auto finish1 = std::chrono::high_resolution_clock::now();
						bwDuration = frameCount * 33 + playStartTime + totalDelay;
						bwIglobal = 0;
						while (bwDuration > byteVec[bwIglobal]) {
							bwIglobal++;
						}
						...
						bwTimeNeeded =  (byteVec[bwIglobal + packetNeeded] - byteVec[bwIglobal]); //in ms
						cout << "dataPos: " << namePos << " name:" << chunkName[namePos] << " size:" << chunkSizeKB[namePos] <<" packTneeded: "<<packetNeeded<<" bwTimeNeeded: "<<bwTimeNeeded<<" bwDivide: "<< chunkBwDivider<< endl;
						timePassed = 0;
						//cout << "time: " << bwDuration << " Index4Time= " << bwIglobal << " reqTime=" << bwTimeNeeded << endl;
						...
						//futures.push_back(std::async(std::launch::async, DownLoadTilesChunk, filename, reqChunkN, chunkD, i, fps, bwTimeNeeded));
						std::async(std::launch::async, DownLoadTilesChunk, filename, reqChunkN, chunkD, i, fps, bwTimeNeeded/10);
					}
					if (tileBitMap2[i] == 0)
					{
						filename = getChunkNametoReq4mTiles(srcBaseAddr2, reqChunkN, i);
						std::ostringstream oss1;
						oss1 << filename << ".avi";
						filename = oss1.str();
						cout << filename << endl;
			...
			float tpTime = reqTiles[reqChunkN].size() * 49+ (24-reqTiles[reqChunkN].size()) * 20;
		...
		if (playIndex == (fps * chunkD - 1))
		{
			if (33 * chunkD * fps < tileProcessT)
			{
				...
				for (int i = 0; i < f; i++)
				{
					blankPixels.push_back(1000);  //this 
					frameRate.push_back(0);   //and this only for figure 10 other wise comment
					//frameCount++;
		...
		eri.ERI2Conv4tilesWithSrFrDelayNewModels(convPixels, frameQvecTiles, bitMapVec[currentChunk], path1.cams[cam_index], tileColN, tileRowN, currentChunk, playIndex, blankP, sravG, srmiN);
		//displayImage(convPixels);
	...
	ofstream output;
	output.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/tile_BP_FR_SR_detailed.txt", std::ios::out | std::ios::app);
	...
	for (int i = 0; i < frameRate.size(); i++)
	{
		...
		totalSR = totalSR + samplingRateAvg[i];
	...
	float SR = totalSR / (float)nonZeroSRCnt;
	output.close();
	...
	ofstream output2;
	output2.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/tile_Size.txt", std::ios::out | std::ios::app);

	for (int i = 0; i < totalSize.size(); i++)
	{
		totalSizeInt = totalSize[i] + totalSizeInt;
	}

	for (int i = 0; i < totalSizeL.size(); i++)
	{
		totalSizeLInt = totalSizeL[i] + totalSizeLInt;
	}
	 
	output2 << srcBaseAddr << bwLog << hmdFileName << "total chunk," << totalSize.size() << "totalSize," << totalSizeInt << "total L chunk," << totalSizeL.size() << "totalSizeL," << totalSizeLInt<< endl;
	output2.close();

	ofstream output1;
	output1.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/FoVP&360Data.txt", std::ios::out | std::ios::app);
	output1 << srcBaseAddr << bwLog << hmdFileName << "," << "Delay," << totalDelay << "SR," << SR << "FrameR," << (mxChunkN*30 ) / (float)(mxChunkN  + totalDelay/1000.0f) << endl;
	output1.close();
	ofstream output3;
	output3.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/Name_tile_requested.txt", std::ios::out | std::ios::app);
	
	for (int i = 0; i < chunkNames.size(); i++)
	{
		output3 << chunkNames[i] << endl;
	}
	output3.close();

	for (int i = 0; i < tilesCol * tilesRow; i++)
	{
		vector<vector <Mat>> temp11;
		for (int chunk = 0; chunk <= mxChunkN + 1; chunk++)
		{
			vector <Mat> temp;
			for (int i = 0; i < (fps * chunkD); i++)
			...
			temp11.push_back(temp);
		}
		frameQvecTiles.push_back(temp11);
	}
}

void testTileFoV2QL(char* srcBaseAddr, char* srcBaseAddr2, char* bwLog, char* size, char* hmdFileName, int nextDlChunkSec, int rowM, int colN, int tileModel)
{
	int fovPlusOrno = tileModel;   //1 fovPlus+360, 0 fov+360,
	float corePredictionMargin = 1;
	int mxChunkN = 59;
	...
	if (!file1)
	{
		print("error: can't open file: " << size << endl);		system("pause");
	}
	string line1;	int i = 0;
	while (getline(file1, line1))
	{
	...
	chunkBwDivider = 5 + 7 * 14 / 34;
	corePredictionMargin = 1;
	...
	getTilesNumber2req2QL(tileBitMap, camera4req, eri, tileColN, tileRowN);
	...
	while (tileCondition)
	{
		...
		if ((playIndex) == 1 && currentChunk < mxChunkN)
		{
			...
			getTilesNumber2req2QL(tileBitMap2, camera4req, eri, tileColN, tileRowN);
		...
		float blankP, srmiN, sravG = 0;
		Mat convPixels(camera2.h, camera2.w, firstScene.type());
		eri.ERI2Conv4tilesWithSrFrDelay2QL(convPixels, frameQvecTiles, bitMapVec[currentChunk], path1.cams[cam_index], tileColN, tileRowN, currentChunk, playIndex, blankP, sravG, srmiN);
		//displayImage(convPixels);
		blankPixels.push_back(blankP);
		frameRate.push_back(30);
		samplingRateAvg.push_back(sravG);
	...
	ofstream output1;
	output1.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/2qlData.txt", std::ios::out | std::ios::app);
	output1 << srcBaseAddr << bwLog << hmdFileName << "," << "Delay, " << totalDelay << " SR," << SR << " FrameR," << (mxChunkN * 30) / (float)(mxChunkN + totalDelay / 1000.0f) << endl;
	output1.close();
	ofstream output3;
	output3.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/Name_tile_requested.txt", std::ios::out | std::ios::app);
...
void testDownloadVideoHttpTileDelayMeasurementFoVPlus(char* srcBaseAddr, char* bwLog, char* size, char* hmdFileName, int nextDlChunkSec, int rowM, int colN, int FoVorNot)
{
	int fovPlusOrno = FoVorNot;   //1 for fovPlus, 0 for fov
	float corePredictionMargin = 1;
	int mxChunkN = 59;
	float accuracy = 100.0f;  //prediction accuracy
	int FrameN2PredictAhead = 0;
	...
	while (tileCondition)
	{
		...
		{			
			if(33*chunkD*fps<tileProcessT)
			{
				...
				for (int i = 0; i < f; i++)
				{
					//blankPixels.push_back(100);  //this 
					frameRate.push_back(0);   //and this only for figure 10 other wise comment
					//frameCount++;
					samplingRateMin.push_back(0);
					samplingRateAvg.push_back(0);
	...
	int srCount = 0;
	for (int i = 0; i < samplingRateAvg.size(); i++)
	{
		totalSR = totalSR + samplingRateAvg[i];
		if (samplingRateAvg[i] != 0)
		{
			srCount++;
		}
	}
	...
	ofstream output2;
	output2.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/tile_Size_concise.txt", std::ios::out | std::ios::app);

	for (int i = 0; i < totalSize.size(); i++)
	{
		totalSizeInt = totalSize[i]+totalSizeInt;
		cout << totalSize[i]<<endl;
		//output2 << totalSize[i] << endl;
	}

	
	output2 << srcBaseAddr << bwLog << hmdFileName << "total chunk, " << totalSize.size() << " totalSize, " << totalSizeInt << endl;
	output2.close();

	ofstream output1;
	output1.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/FoVPdata.txt", std::ios::out | std::ios::app);
	cout<< "Delay OldWa," << totalDelay <<",bp,"<<bp<<endl;
	
	//output1 <<srcBaseAddr<<bwLog<<hmdFileName<<": "<< "Delay: " << totalDelay << ",bp:" << bp<<", totalTileChunk:"<<totalTile<<" Fravg:"<<(chunkN*30)/(float)(chunkN+totalDelay)<< endl;
	output1 << srcBaseAddr << bwLog << hmdFileName << "," << "Delay," << totalDelay<<" SR,"<<SR << "FrameR," << (mxChunkN * 30*1000) / (float)(mxChunkN*1000 + totalDelay) <<" 1MP%,"<<(a1MP*100)/(float)blankPixels.size() <<" MMP,"<<MMP<< endl;
	output1.close();
...
void testDownloadVideoHttpTileQualChngLineMeasurementFoVPlus(char* srcBaseAddr, char* bwLog, char* chunkSizes, char* hmdFileName, int nextDlChunkSec, int rowM, int colN)
{
	int fovOnly = 1;
	int mxChunkN = 35;
	float corePredictionMargin = 1;
	...
	float f1ap = float(a1p*100.0f / totalCount);
	ofstream output1;
	output1.open("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/mobisys/qualChangeData.txt", std::ios::out | std::ios::app);
	output1 << srcBaseAddr << bwLog << hmdFileName << "," << "1ap(%)," << f1ap << ",map," <<map<< endl;
	output1.close();
...
int Writeh264VideoUptoNframes() {
	...
	for (int i = 1; i < 2; i++)	
	{
		path1.WriteH264_1min("./Video/source/1s4s/ny.mkv", i, 60, codec);		
		path1.WriteH264_1min("./Video/source/1s4s/paris.mkv", i, 60, codec);
		path1.WriteH264_1min("./Video/source/1s4s/elephant.webm", i, 60, codec);
	}
	/*
	for (int i = 1; i < 61; i++)
	{
		path1.WriteH264("./Video/source/1s4s/ny.mkv", i, 1, codec);		
		path1.WriteH264("./Video/source/1s4s/paris.mkv", i, 1, codec);
		path1.WriteH264("./Video/source/1s4s/elephant.webm", i, 1, codec);
	}
	*/
	return 0;
}
...
int testWriteh264tiles(char* fileNamex) {
	Path path1;
	int codec = VideoWriter::fourcc('H', '2', '6', '4');

	vector<double>encodeTime;

	for (int i = 1; i < 5; i++)
	{
		auto start = std::chrono::high_resolution_clock::now();

		path1.WriteH264tiles(fileNamex, i, 6, 4, codec);

		auto finish1 = std::chrono::high_resolution_clock::now();
		std::chrono::duration<double> elapsed1 = finish1 - start;
		encodeTime.push_back(elapsed1.count());
		cout << "time.........: " << elapsed1.count() << endl;
	}

	ofstream output("./Video/source/encTimetile.txt");

	for (int i = 0; i < encodeTime.size(); i++)
	{
		output << encodeTime[i] * 1000 << endl;
	}
...
void GenerateEncoding4sVarSpecificPanTiltChunk(char* fileNamex)
{	
	Path path1;
	float var[10];
	ifstream  f("C:/Users/pmija/OneDrive - purdue.edu/lab129/EdgeVR/Code/Video/source/4kDivingEncodingVariable.txt");
	...
	//char* fileNamex = "./Video/source/originalSources30crf/elephant/elephant.webm";			

	
	for (int chunkN = 1; chunkN < 2; chunkN++)
	{

		EncodeVideoWithExtensionSpecificPanTiltChunkN(fileNamex, extraSec, chunkN, extraFrame, extraSec);    //change prediction margin inside//*************//
	}
...
void EncodeVideoWithExtensionSpecificPanTiltChunkN(char* fileName, int extraSec, int chunkN, int extraFrame, int howManySecondsfor4thSec)
{
	chunkN = chunkN;
	...
	for (int tilt = -90; tilt <= 90; tilt = tilt + 20)//
	{
		...
		ofstream output("./Video/source/encTime.txt");
...
void GenerateEncoding4AllDirection(char* fileNamex)
{
	
	GenerateEncoding4sVarSpecificPanTiltChunk(fileNamex);
...
int out_video_file()//(Mat &output_image_mat, ERI eri_image, Path path1)
{
	...
	VideoCapture cap("http://127.0.0.5:80/3vid2crf3trace/android/tilt0/30_rhino.webm4_20_0_0.avi.mp4");
	if (!cap.isOpened()) {
		cout << "Cannot open the video file" << endl;
		system("pause");

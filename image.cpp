...
void DownLoadChunk4thSecVar(string filename, int chunkD, int fps)
{
	...
	while (condition)
	{
		...
		if (decodeCount <= 90) 
		{
			...
			if (decodeCount == 60)
			{
				...
				NextChunkDownloaded = 0;
				futures.push_back(std::async(std::launch::async, DownLoadChunk4thSecVar, filename, chunkD, fps));
